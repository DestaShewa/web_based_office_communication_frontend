...
import {
  LogOut, LayoutDashboard, ListChecks, Users,
  Bell, MessageSquare, Menu, X, Calendar, UserCircle, Key, FileText, Mail
} from 'lucide-react';
...
export default function CoordinatorLayout() {
  ...
  const navLinks = [
    { name: 'Dashboard', path: '/coordinator', icon: <LayoutDashboard size={20} /> },
    { name: 'Tasks', path: '/coordinator/tasks', icon: <ListChecks size={20} /> },
    { name: 'Announcements', path: '/coordinator/announcements', icon: <Bell size={20} /> },
    { name: 'Meetings', path: '/coordinator/meetings', icon: <Calendar size={20} /> },
    { name: 'Messages', path: '/coordinator/messages', icon: <MessageSquare size={20} /> },
    { name: 'Official Memos', path: '/coordinator/memos', icon: <FileText size={20} /> },
  ];
      ...
      >
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
          <div className="truncate w-full pr-2">
            <p className="text-sm font-semibold truncate text-primary">{user?.name || 'Coordinator'}</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[9px] sm:text-[10px] font-bold text-accent uppercase tracking-widest truncate">
                {user?.office ? 'Office' : 'Department'} Oversight
              </span>
            </div>
          </div>
