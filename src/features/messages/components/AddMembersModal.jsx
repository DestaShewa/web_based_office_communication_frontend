...
import { addGroupMembers } from '../api/messageMutations';
import useAuthStore from '../../../store/authStore';
import { getAvatarUrl } from '../utils/statusUtils';
import { toast } from 'sonner';

export default function AddMembersModal({ groupId, existingMemberIds, onClose, triggerRect }) {
  ...
  const { data: usersData, isLoading } = useQuery({
    queryKey: ['users', 'add-members', userFacultyId, userDeptId],
    queryFn: () => getUsers({
  ...
  const filteredUsers = users.filter(u => 
    ...
    !existingMemberIds.includes(u.customId)
  );
  ...
  return (
    <div className="fixed inset-0 z-[500] overflow-hidden pointer-events-none">
      ...
      <div 
        ref={menuRef}
        className="fixed bg-white w-full max-w-[340px] rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 overflow-visible flex flex-col pointer-events-auto animate-in zoom-in-95 duration-200"
        style={{
          top: position.top,
          left: position.left,
          ...
          maxHeight: position.top ? `calc(100vh - ${position.top}px - ${position.padding || 24}px)` : '80vh'
        }}
      ...
      >
        ...
        <div className="p-6 pb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-black text-primary leading-tight">Add Members</h2>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Select staff to add</p>
          </div>
          <button 
            onClick={onClose}
              ...
              filteredUsers.map(u => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 p-2 rounded-xl transition-all text-left group border ${
                      isSelected 
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[9px] text-gray-400 font-bold uppercase truncate tracking-tighter">
                        {u.role} • {u.department?.name || u.department || 'General'}
                      </p>
                    </div>
                  </button>
                );
