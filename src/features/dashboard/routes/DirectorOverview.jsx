...
export default function DirectorOverview() {
  ...
  return (
    <div className="space-y-6">
      {/* ─── Header ────────────────────────── */}
      <div>
        <h2 className="text-2xl font-bold text-primary">Director's Dashboard</h2>
        <p className="text-gray-500">Real-time institutional oversight and reports.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard title="Total Personnel" value={overview.totalUsers} isLoading={isLoading} icon={<Users size={24} />} colorClass="bg-blue-600" subtitle="Organization size" breakdown={overview.roleBreakdown} />
        <StatCard title="Active Assignments" value={overview.activeTasks} isLoading={isLoading} icon={<ListChecks size={24} />} colorClass="bg-orange-500" subtitle={`${taskStats.overdue} overdue items across org`} />
        <StatCard title="Institution Meetings" value={overview.upcomingMeetings} isLoading={isLoading} icon={<Calendar size={24} />} colorClass="bg-green-600" subtitle="Scheduled ahead" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:col-span-1 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-primary flex items-center gap-2"><BarChart3 size={20} className="text-orange-500" /> Institutional Delivery</h3>
          </div>
          ...
          {taskLoading ? (
            ...
            <div className="space-y-5 flex-1 flex flex-col justify-between">
              ...
              <button onClick={() => navigate('/director/tasks')} className="w-full flex items-center justify-center gap-2 text-xs font-bold text-primary bg-gray-50 hover:bg-gray-100 py-2.5 rounded-lg transition-colors mt-2">
                Launch Task Viewer <ArrowRight size={14} />
              </button>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:col-span-2 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-primary flex items-center gap-2"><Calendar size={20} className="text-green-500" /> Upcoming Meetings</h3>
            <button onClick={() => navigate('/director/meetings')} className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 transition-colors">
          ...
          {mtgLoading ? (
                ...
                <div key={mtg.customId || idx} className="flex items-start gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${mtg.status === 'scheduled' ? 'bg-blue-100 text-blue-600' :
                        mtg.status === 'in_progress' ? 'bg-amber-100 text-amber-600' :
                          'bg-gray-100 text-gray-500'
                        }`}>{mtg.status || 'scheduled'}</span>
                      {mtg.participants && <span className="text-[10px] text-gray-400">{mtg.participants.length} delegates expected</span>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-12 text-gray-300">
              ...
              <p className="text-xs text-gray-300 mt-1">No upcoming meetings scheduled.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-primary flex items-center gap-2"><Megaphone size={20} className="text-amber-500" /> Institutional Announcements</h3>
          <button onClick={() => navigate('/director/announcements')} className="text-xs font-bold text-primary hover:text-accent flex items-center gap-1 transition-colors">
            View All <ChevronRight size={14} />
          </button>
        </div>
        ...
        {annLoading ? (
            ...
            {announcements.slice(0, 3).map((ann, idx) => (
              <div key={ann.customId || idx} className="p-4 rounded-xl hover:bg-gray-50 transition-colors border border-gray-50 hover:border-gray-200 group">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`shrink-0 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg mt-0.5 shadow-sm ${ann.priority === 'urgent' ? 'bg-red-50 text-red-600 border border-red-100' :
                ...
                <div className="flex items-center gap-3 mt-3 text-[10px] text-gray-400 font-medium">
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 rounded-full bg-gray-200 flex items-center justify-center text-[8px] font-bold text-gray-500 uppercase">
                      {ann.createdBy?.name?.charAt(0) || ann.author?.name?.charAt(0) || 'C'}
                    </div>
                    <span>{ann.createdBy?.name || ann.author?.name || 'Central Auth'}</span>
                  </div>
                  <span>·</span>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-gray-300">
            ...
            <p className="text-xs text-gray-300 mt-1 uppercase tracking-widest font-medium">No recent announcements.</p>
          </div>
        )}
      </div>
    </div>
